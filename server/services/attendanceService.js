...
// The logic there is that if there is a date like today's date it has been marked else not
  ...
  for(let i=0;i<classStudents.length;i++){
    ...
    const data = {"_id": _id, "class": class_, "user":user}
...
const getStudentsPresent = async()=>{
  ...
  return {studentsPresent: studentsPresent, NoOfStudents: studentsPresent.length}
}
...
// The logic there is that if there is a date like today's date it has been marked else not
  ...
  if (isItToday) {
    ...
    for(let i=0;i<attendanceData.length;i++){
      ...
      const data = {student: student_id, isPresent: isPresent, markedBy:teacher_id, markedAt: today}
      console.log(data)
